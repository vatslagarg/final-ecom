...
function applycoupanCode(){
  jQuery('#coupan_code_msg').html('');
  jQuery('#order_place_msg').html('');

  var coupan_code=jQuery('#coupan_code').val();
  if(coupan_code!=''){
    jQuery.ajax({
      type:'post',
      url:'/apply_coupan_code',
      data:'coupan_code='+coupan_code+'&_token='+jQuery("[name='_token']").val(),
      success:function(result){
        console.log(result.status);
        if(result.status=='success'){
          jQuery('.show_coupan_box').removeClass('hide');
          jQuery('#coupan_code_str').html(coupan_code);
          jQuery('#total_price').html('INR '+result.totalPrice);
          jQuery('.apply_coupan_code_box').hide();
        }else{
          
        }
        jQuery('#coupan_code_msg').html(result.msg);
      }
    });
  }else{
    jQuery('#coupan_code_msg').html('Please enter coupan code');
  }
}

function remove_coupan_code(){
  jQuery('#coupan_code_msg').html('');
  var coupan_code=jQuery('#coupan_code').val();
  jQuery('#coupan_code').val('');
  if(coupan_code!=''){
    jQuery.ajax({
      type:'post',
      url:'/remove_coupan_code',
      data:'coupan_code='+coupan_code+'&_token='+jQuery("[name='_token']").val(),
      success:function(result){
        if(result.status=='success'){
          jQuery('.show_coupan_box').addClass('hide');
          jQuery('#coupan_code_str').html('');
          jQuery('#total_price').html('INR '+result.totalPrice);
          jQuery('.apply_coupan_code_box').show();
        }else{
