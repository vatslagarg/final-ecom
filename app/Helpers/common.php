...
function apply_coupan_code($coupan_code){
	$totalPrice=0;
	$result=DB::table('coupans')  
		->where(['code'=>$coupan_code])
		->get(); 
	...
	if(isset($result[0])){
		...
		if($result[0]->status==1){
			...
			$status="error";
			$msg="coupan code deactivated";   
		}
		
	}else{
		$status="error";
		$msg="Please enter valid coupan code";
	}
	
	$coupan_code_value=0;
	if($status=='success'){
		if($type=='Value'){
			$coupan_code_value=$value;
			$totalPrice=$totalPrice-$value;
		}if($type=='Per'){
			$newPrice=($value/100)*$totalPrice;
			$totalPrice=round($totalPrice-$newPrice);
			$coupan_code_value=$newPrice;
		}
	}

	return json_encode(['status'=>$status,'msg'=>$msg,'totalPrice'=>$totalPrice,'coupan_code_value'=>$coupan_code_value]);
}
