...
class FrontController extends Controller
{
    ...
    {
         $arr=apply_coupan_code($request->coupan_code);
         $arr=json_decode($arr,true);

         return response()->json(['status'=>$arr['status'],'msg'=>$arr['msg'],'totalPrice'=>$arr['totalPrice']]);
    }
    
    public function remove_coupan_code(Request $request)
    {
        $totalPrice=0;
        $result=DB::table('coupans')  
        ->where(['code'=>$request->coupan_code])
        ->get(); 
        ...
        return response()->json(['status'=>'success','msg'=>'coupan code removed','totalPrice'=>$totalPrice]); 
    }
    ...
    public function place_order(Request $request)
    {
        ...
        $coupan_value=0;
        if($request->coupan_code!=''){
            $arr=apply_coupan_code($request->coupan_code);
            $arr=json_decode($arr,true);
            if($arr['status']=='success'){
                $coupan_value=$arr['coupan_code_value'];
            }else{
        ...
        if($order_id>0){
            ...
            if($request->payment_type=='Gateway'){
                $final_amt=$totalPrice-$coupan_value;
                $ch = curl_init();
                ...
                curl_setopt($ch, CURLOPT_HTTPHEADER,
                    array("X-Api-Key:test_a4050c894bac743f6d1a4cbc240",
                    "X-Auth-Token:test_12c0327329e2d9222a91443b39e"));
                $payload = Array(
                    'purpose' => 'Buy Product',
                    'amount' => $final_amt,
